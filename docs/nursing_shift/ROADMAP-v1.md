...
**Correção de status HTTP (2026-07-28):** `authorize_user_request` retornava `403 Forbidden` tanto para token ausente/inválido/expirado quanto para usuário inexistente ou inativo — misturando "não autenticado" com "autenticado mas sem permissão". Isso quebrava silenciosamente a recomendação do próprio guia de frontend ("ao receber 401, tente refresh"), porque um token expirado nunca gerava 401. Corrigido: token ausente/inválido/expirado/usuário inexistente → `401 Unauthorized`; usuário inativo → `403 Forbidden` (mantido, pois refresh não resolve esse caso). Descoberto ao depurar um caso real onde o frontend não conseguia criar um usuário com um `maintainer` logado — o token de acesso (TTL de 15 min) já havia expirado e o frontend não tinha como saber que devia tentar refresh.

**Exclusão e inativação de usuário (2026-07-28):** adicionados `DELETE /api/users/:id` e `PUT /api/users/:id/inactivate`, ambos restritos a `administrator`/`maintainer` e bloqueados contra a própria conta (guarda nova `forbid_self_target`, diferente da checagem de grupo já existente). A regra "não pode zerar administrator/maintainer ativos" (antes só em `ChangeUserGroup`) foi extraída para `Arkham::UseCases::EnsurePrivilegedGroupRemains` e reaproveitada nos três use cases — nos dois novos ela é defesa em profundidade (não alcançável via API hoje, já que quem executa a ação sempre precisa ser um privilegiado diferente do alvo). Constante `PRIVILEGED_GROUPS` também centralizada em `User::PRIVILEGED_GROUPS` (antes duplicada no controller e no use case). Sem endpoint de reativação — inativação é unidirecional via API por enquanto.

**Tarefas futuras identificadas (não fazem parte deste marco):**
- [ ] Endpoint de reativação de usuário (`PUT /api/users/:id/activate` ou equivalente) — hoje só é possível via banco de dados
