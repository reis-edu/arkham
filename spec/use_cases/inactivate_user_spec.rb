  ...
  describe '#execute' do
    context 'when the user exists and the safety check passes' do
      before do
        ...
        allow(user_repository).to receive(:inactivate).with(user_id).and_return(user_id)
      end

      it 'inactivates the user' do
        expect(user_repository).to receive(:inactivate).with(user_id)
        use_case.execute(user_id)
      end

      it 'returns the user id' do
        expect(use_case.execute(user_id)).to eq(user_id)
      end
    end
    ...
    context 'when the user does not exist' do
      ...
      it 'raises UserNotFoundError and does not inactivate anything' do
        expect(user_repository).not_to receive(:inactivate)
