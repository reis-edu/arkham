...
RSpec.describe Arkham::Core::UseCases::InactivatePatient do
  ...
  describe '#execute' do
    ...
    context 'when patient exists' do
      before do
        ...
        allow(patient_repository).to receive(:inactivate)
      end

      it 'inactivates the patient' do
        expect(patient_repository).to receive(:inactivate).with(patient_id)
    ...
    context 'when repository fails' do
      before do
        allow(patient_repository).to receive(:find_by_id).with(patient_id).and_return(double('Patient'))
        allow(patient_repository).to receive(:inactivate).and_raise(StandardError.new('Database error'))
      end
